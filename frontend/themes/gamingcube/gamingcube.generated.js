...
window.Vaadin.Flow['_vaadintheme_gamingcube_globalCss'] = window.Vaadin.Flow['_vaadintheme_gamingcube_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_gamingcube_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_gamingcube_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_gamingcube_globalCss'].push(target);
  }
  if (!document['_vaadintheme_gamingcube_componentCss']) {
    registerStyles(
    ...
    document['_vaadintheme_gamingcube_componentCss'] = true;
  }
